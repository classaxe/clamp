...
unless File.exist?("#{dir}/config.yml")
  puts "\n" +
    ...
    '********************' + "\n" +
    "\n" +
    "\e[31mERROR: Configuration file not found.\e[0m" + "\n" +
    "\n" +
    "Please copy example.config.yml to config.yml and try again." + "\n" +
    "\n"
  exit
end
...
mounts_required.each do |required_folder|
  ...
  if found == false
    puts "\n" +
      ...
      '********************' + "\n" +
      "\n" +
      "\e[31mERROR: Your config.yml file must contain " + mounts_required.count.to_s + " vagrant synced folder entries" + "\n" +
      "mapping to " + mounts_required.to_s + "\e[0m" + "\n" +
      "\n" +
