...
if [ "${install_solr}" = 'y' ]; then
    ...
    if [ ! -d /opt/solr ]; then
        cd /opt/
        wget --progress=bar:force https://github.com/classaxe/clamp-assets/releases/download/v1.0/solr.tar.gz
    ...
    if [ ! -f /etc/apache2/sites-available/100-solr.dev.classaxe.com.conf ]; then
        # Add Vhost
        sudo tee /etc/apache2/sites-available/100-solr.dev.classaxe.com.conf <<EOL
<VirtualHost *:8090>
  ProxyPreserveHost On
  ProxyRequests Off
  ServerName solr.dev.classaxe.com
...
EOL
        ...
        # Enable Vhost
        sudo a2ensite 100-solr.dev.classaxe.com
